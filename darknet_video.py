...
def set_saved_video(input_video, output_video, size):
    ...
    #fps = int(input_video.get(cv2.CAP_PROP_FPS))colormap
    video = cv2.VideoWriter(output_video, fourcc, 30, (640,480),1)
    return video

def video_capture(frame_queue, darknet_image_queue, depth_queue):
    while True:
        cap = pipeline.wait_for_frames() 
        depth_frame = cap.get_depth_frame() ###
        color_frame  = cap.get_color_frame()
...
if __name__ == '__main__':
    ...
    Thread(target=video_capture, args=(frame_queue, darknet_image_queue,depth_queue)).start()
